...
def prepare_prompt(question:pd.DataFrame, few_n:int=3):
    ...
    context =  """Owner_ID -- Unique ID of each owner of the property; Owner_First_Name -- First name of the owner of the property ; Owner_Family_Name -- Family name of the owner of the property ; Property_Type -- Specific type of the property given in Italian. For example, "casa", "bottega da barbier", "bottega da fruttariol". ; Rent_Income -- Rent price of the property that the owner receives as income, given in Venice ancient gold coin ducato. ; Property_Location -- Ancient approximate toponym of the property given in Italian."""
    for idx, val in enumerate(top_n_idx):
        few_shot = '\n'.join([table_metadata+'\n'+context+'\n'+demonstration.iloc[i]['question']+'\n'+demonstration.iloc[i]['evidence']+'\n'+demonstration.iloc[i]['true_query'] for i in val])
        q = question.iloc[idx]['question']
        e = question.iloc[idx]['evidence']
        in_prompt.append(prompt.format(few_shot=few_shot, table_metadata=table_metadata, context=context, question=q, evidence=e))
