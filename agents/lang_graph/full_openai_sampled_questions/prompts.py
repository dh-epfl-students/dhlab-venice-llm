...
extract_column_prompt = """Given a question, you need to match the phrases in the question with the columns in the dataset if applicable. Only focus on the phrases that refer to one or more columns in any of the above datasets. If none of the phrases refer to a specific dataset column, return an empty list.
...
extract_row_prompt = """You are a given a mapping between a phrase and a column of a dataset. Your task is to hypothesise if the given phrase could correspond to a specific value in the matching column depending on the definition and data type of what should be given in the columns.
...
info_prompt = """Instruction:
You will be given a question, the assistant's code to answer the question, and the final output. Please modify the last part of the code to print the number of rows of the final dataset used to get the final output. And save the final dataset to the following path "info_data/info_{index}.csv"
